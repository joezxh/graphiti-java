...
1. **类型系统**(Type System)
   - 定义知识图谱中有哪些类型的实体(如Party当事人、Mediator调解组织、Dispute纠纷)
   - 定义实体之间的关系类型(如DISPUTE_PARTY当事人参与、DISPUTE_MEDIATOR纠纷审理)
   - **作用**: 保证社会治理数据的一致性和规范性

2. **验证框架**(Validation Framework)
   - 必填属性检查(如Party必须有partyName当事人姓名)
   - 数据类型检查(如Dispute.filingDate必须是日期)
   - 约束规则(如disputeNumber必须符合调解组织编号格式)
   - **作用**: 在数据写入前拦截错误

3. **推理基础**(Reasoning Foundation)
   - 类层次推理(如Plaintiff申请人是Party的子类)
   - 属性推断(如DISPUTE_PARTY的逆关系是CASE_HAS_PARTY)
   - 一致性检查(如某实体不能既是Party又是Mediator)
   - **作用**: 从已有社会治理知识推导新知识
...
```java
// 概念 (Concepts) - 社会治理实体类
Class: Party, Mediator, Dispute, LegalProvision, Mediator, Evidence

// 关系 (Relations) - 社会治理关系
Property: 
  - DISPUTE_PARTY(Party → Dispute)         // 当事人参与纠纷
  - DISPUTE_MEDIATOR(Dispute → Mediator)         // 纠纷由调解组织审理
  - DISPUTE_MEDIATOR(Dispute → Mediator)         // 调解员审理纠纷
  - CASE_GOVERNANCE_BASIS(Dispute → LegalProvision)  // 纠纷适用法条
  - MEDIATION_AGREEMENT(Dispute → Dispute)       // 上诉关系

// 公理 (Axioms) - 社会治理约束
Constraint:
  - Dispute.disputeNumber 符合格式: ^（\d{4}）[\u4e00-\u9fa5]{2,6}民[初终]{1}\d{3,8}号$
  - Party.partyType ∈ ["自然人", "法人", "非法人组织"]
  - Party.partyRole ∈ ["申请人", "被申请人", "第三人", "上诉人", "被上诉人"]
  - Mediator.courtLevel ∈ ["最高人民调解组织", "高级人民调解组织", "中级人民调解组织", "基层人民调解组织"]

// 实例 (Instances) - 存储在Neo4j中,来自真实案例
Individual:
  - 张某新 (type: Party, partyRole: 申请人, partyType: 自然人)
  - 某网络借贷平台 (type: Party, partyRole: 被申请人, partyType: 法人)
  - 网贷利率纠纷案 (type: Dispute, disputeNumber: SZ2024-0001)
  - 深圳市南山区人民调解委员会 (type: Mediator, courtLevel: 基层人民调解组织)
  - 《民法典》第680条 (type: LegalProvision)
```
...
LegalEntity (社会治理实体 - 根类)
  └─ Party (当事人)
      ...
      └─ LegalPerson (法人)
          └─ Company (公司)
  └─ Mediator (调解组织)
      ├─ SupremeMediator (最高人民调解组织)
      └─ LocalMediator (地方调解组织)
  └─ Dispute (纠纷)
      ├─ CivilDispute (民事纠纷)
      └─ CriminalDispute (刑事纠纷)
```

**实际案例**: 张某新与某网络借贷平台网贷利率纠纷案
- 张某新 → type: NaturalPerson (自然人当事人)
- 某网络借贷平台 → type: LegalPerson (法人当事人)
- 深圳市南山区人民调解委员会 → type: Mediator (调解组织)
- 网贷利率纠纷案 → type: CivilDispute (民事纠纷)
...
| class_uri | VARCHAR(512) | 完整URI | `http://social-gov.cc/ontology#Person` |
...
| example | TEXT | 使用示例 | "张某新、某网络借贷平台" |
| domain_hint | VARCHAR(32) | 领域分类标记 | "社会治理主体" |
...
**创建类**(curl) - 社会治理领域示例:
```bash
# 创建社会治理根类: LegalEntity
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/classes' \
  -H 'Authorization: Bearer <token>' \
  -H 'Content-Type: application/json' \
  -d '{
    "localName": "LegalEntity",
    "classUri": "http://social-gov.cc/ontology#LegalEntity",
    "description": "社会治理领域实体的顶层抽象类",
    "domainHint": "GOVERNANCE"
  }'
...
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/classes' \
  ...
  -d '{
    "localName": "Party",
    "classUri": "http://social-gov.cc/ontology#Party",
    "parentClassId": 1,
    "description": "纠纷中的当事人,包括自然人、法人和非法人组织。",
    "example": "{\"partyName\": \"张某新\", \"partyType\": \"自然人\", \"partyRole\": \"申请人\"}",
    "domainHint": "GOVERNANCE"
  }'
```
...
```json
{
  ...
  "data": {
    ...
    "localName": "Party",
    "parentClassId": 1,
    "parentClassUri": "http://social-gov.cc/ontology#LegalEntity",
    "description": "纠纷中的当事人,包括自然人、法人和非法人组织。",
    "example": "{\"partyName\": \"张某新\", \"partyType\": \"自然人\", \"partyRole\": \"申请人\"}",
    "domainHint": "GOVERNANCE",
...
```sql
-- PostgreSQL 社会治理本体类定义示例
INSERT INTO ont_class (id, definition_id, class_uri, local_name, parent_class_id, description, example, domain_hint) VALUES
-- 社会治理根类
(5, 1, 'http://social-gov.cc/ontology/LegalEntity', 'LegalEntity', NULL,
 '社会治理领域实体的顶层抽象类', NULL, 'GOVERNANCE'),

-- 核心社会治理实体类
(10, 1, 'http://social-gov.cc/ontology/Party', 'Party', 5, 
 '纠纷中的当事人,包括自然人、法人和非法人组织。',
 '{"partyName": "张某新", "partyType": "自然人", "partyRole": "申请人"}',
 'GOVERNANCE'),

(20, 1, 'http://social-gov.cc/ontology/Mediator', 'Mediator', 5,
 '调解组织,包括最高人民调解组织、高级人民调解组织、中级人民调解组织、基层人民调解组织。',
 '{"courtName": "深圳市中级人民调解组织", "courtLevel": "中级人民调解组织"}',
 'GOVERNANCE'),

(30, 1, 'http://social-gov.cc/ontology/Dispute', 'Dispute', 5,
 '社会治理诉讼纠纷,包括民事、刑事、行政纠纷。',
 '{"caseName": "网贷利率纠纷案", "disputeNumber": "SZ2024-0001"}',
 'GOVERNANCE'),

(40, 1, 'http://social-gov.cc/ontology/Mediator', 'Mediator', 5,
 '纠纷审判人员,包括审判长、审判员、人民陪审员。',
 '{"judgeName": "张某", "judgeTitle": "审判长"}',
 'GOVERNANCE'),

(50, 1, 'http://social-gov.cc/ontology/LegalProvision', 'LegalProvision', 5,
 '社会治理条文,包括社会治理、法规、司法解释的具体条款。',
 '{"lawName": "中华人民共和国民法典", "articleNumber": "第182条"}',
 'GOVERNANCE');
```
...
| 类型 | 英文名称 | 连接对象 | Neo4j存储 | 社会治理领域示例 |
|------|---------|---------|-----------|------|
| **对象属性** | Object Property | 类 → 类 | **Edge(边)** | DISPUTE_PARTY(Party→Dispute) |
...
**示例** (社会治理知识图谱):
```
DISPUTE_PARTY (对象属性 - 纠纷当事人关系)
  Domain: Party (只能是当事人"参与纠纷")
  Range: Dispute (参与的对象是纠纷)
  
partyName (数据属性 - 当事人姓名)
  Domain: Party
  Range: xsd:string (字符串)

disputeNumber (数据属性 - 纠纷编号)
  Domain: Dispute
  Range: xsd:string (字符串, 格式:（年份）调解组织简称+纠纷类型+编号)
```
...
| id | BIGINT | 主键 | 100 |
| definition_id | BIGINT | 所属本体定义ID | 10 |
| property_uri | VARCHAR(512) | 完整URI | `http://social-gov.cc/ontology#WORKS_AT` |
...
**创建数据属性**(Party.partyName) - 社会治理领域示例:
```bash
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/properties' \
  -H 'Content-Type: application/json' \
  -d '{
    "localName": "partyName",
    "propertyUri": "http://social-gov.cc/ontology#hasPartyName",
...
**创建对象属性**(Party.DISPUTE_PARTY Dispute) - 社会治理领域示例:
```bash
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/properties' \
  -H 'Content-Type: application/json' \
  -d '{
    "localName": "DISPUTE_PARTY",
    "propertyUri": "http://social-gov.cc/ontology#hasDisputeParty",
    "propertyType": "OBJECT",
    "domainClassId": 10,
    "rangeClassId": 30,
    "isRequired": true,
    "isMultiple": true,
    "description": "当事人参与纠纷的关系,包括申请人、被申请人、第三人"
  }'
```
...
-- Party 类属性
(101, 1, 'http://social-gov.cc/ontology/hasPartyName', 'partyName', 'DATATYPE', 10, 'string', TRUE,
 '当事人姓名或名称'),
(102, 1, 'http://social-gov.cc/ontology/hasPartyType', 'partyType', 'DATATYPE', 10, 'string', TRUE,
 '当事人类型:自然人/法人/非法人组织'),
(103, 1, 'http://social-gov.cc/ontology/hasPartyRole', 'partyRole', 'DATATYPE', 10, 'string', TRUE,
 '当事人在纠纷中的角色:申请人/被申请人/第三人'),

-- Dispute 类属性
(201, 1, 'http://social-gov.cc/ontology/hasDisputeNumber', 'disputeNumber', 'DATATYPE', 30, 'string', TRUE,
 '纠纷编号,格式:(年份)调解组织简称+纠纷类型+编号'),
(202, 1, 'http://social-gov.cc/ontology/hasDisputeType', 'disputeCategory', 'DATATYPE', 30, 'string', TRUE,
 '纠纷类型:民事纠纷/刑事纠纷/行政纠纷'),
(203, 1, 'http://social-gov.cc/ontology/hasFilingDate', 'filingDate', 'DATATYPE', 30, 'date', FALSE,
 '纠纷受理日期'),

-- Mediator 类属性
(301, 1, 'http://social-gov.cc/ontology/hasMediatorName', 'courtName', 'DATATYPE', 20, 'string', TRUE,
 '调解组织名称'),
(302, 1, 'http://social-gov.cc/ontology/hasMediatorLevel', 'courtLevel', 'DATATYPE', 20, 'string', FALSE,
 '调解组织级别:最高/高级/中级/基层');
```
...
| 类型 | 说明 | 值格式 | 社会治理领域示例 |
|------|------|--------|------|
| **CARDINALITY** | 基数约束 | `min:max` | `1:100`(一个纠纷至少1个当事人,最多100个) |
| **PATTERN** | 正则表达式 | Java Regex | `^（\d{4}）[\u4e00-\u9fa5]{2,6}民[初终]{1}\d{3,8}号$` (纠纷编号) |
| **RANGE** | 数值范围 | `min:max` | `0:10000000000`(争议金额0-100亿) |
| **ENUM** | 枚举约束 | JSON数组 | `["自然人", "法人", "非法人组织"]` (当事人类型) |
| **NOT_NULL** | 非空约束 | `true` | partyName(当事人姓名)必填 |
| **UNIQUE** | 唯一约束 | `true` | disputeNumber(纠纷编号)不重复 |
...
**创建纠纷编号格式约束** (PATTERN) - 社会治理领域示例:
```bash
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/constraints' \
  -H 'Content-Type: application/json' \
  -d '{
    "propertyId": 201,
    "constraintType": "PATTERN",
    "value": "{\"pattern\": \"^（\\\\d{4}）[\\\\u4e00-\\\\u9fa5]{2,6}\\\\u6c11[\\\\u521d\\\\u7ec8]{1}\\\\d{3,8}号$\"}",
    "errorMessage": "纠纷编号格式错误,应为:(年份)调解组织简称+纠纷类型+编号,如(2022)沪0105民初21387号",
    "severity": "ERROR",
    "description": "纠纷编号必须符合中国调解组织标准格式"
  }'
```

**创建当事人类型枚举约束** (ENUM) - 社会治理领域示例:
```bash
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/constraints' \
  -H 'Content-Type: application/json' \
  -d '{
...
-- 3. 当事人角色枚举约束 (ENUM)
(3, 1, 10, 103, 'ENUM',
 '{"allowed_values": ["申请人", "被申请人", "第三人", "上诉人", "被上诉人"]}',
 '当事人角色必须是:申请人、被申请人、第三人、上诉人或被上诉人',
 'ERROR',
 '当事人纠纷角色枚举约束'),

-- 4. 调解组织级别枚举约束 (ENUM)
(4, 1, 20, 302, 'ENUM',
 '{"allowed_values": ["最高人民调解组织", "高级人民调解组织", "中级人民调解组织", "基层人民调解组织", "专门调解组织"]}',
 '调解组织级别必须是:最高人民调解组织、高级人民调解组织、中级人民调解组织、基层人民调解组织或专门调解组织',
 'ERROR',
 '中国调解组织五级体系约束'),
...
示例:
OntProperty: DISPUTE_PARTY (定义域:Party, 值域:Dispute)
   ↓
Edge: (张某新)-[DISPUTE_PARTY {role: "申请人"}]->(网贷利率纠纷案)
...
```
1. 定义本体属性 (关系数据库)
   POST /ontology/social-gov-kg/properties
   {
     "localName": "DISPUTE_PARTY",
     "propertyType": "OBJECT",
     "domainClassId": 10,  // Party
     "rangeClassId": 30    // Dispute
   }

2. 创建实体节点 (Neo4j)
   CREATE (p:Entity {type: "Party", name: "张某新", partyName: "张某新", partyRole: "申请人"})
   CREATE (c:Entity {type: "Dispute", name: "网贷利率纠纷案", disputeNumber: "SZ2024-0001"})

3. 创建关系边 (Neo4j) - 受本体约束
   CREATE (p)-[r:RELATES_TO {
     uuid: "rel-party-case-001",
     type: "DISPUTE_PARTY",
     fact: "张某新作为申请人提起网贷利率纠纷诉讼",
     role: "申请人",
...
4. 验证边是否符合本体
   ...
   - target节点type必须是Dispute(range)
   - 边类型DISPUTE_PARTY必须在ont_property中定义
```
```
...
ValidationResultVO validation = ontologyValidationService.validateEdge(
    graphId, 
    "DISPUTE_PARTY", 
    Map.of("fact", "张某新作为申请人提起网贷利率纠纷诉讼", "role", "申请人")
);
...
**社会治理领域关系边完整示例** (Cypher):

```cypher
// 1. 纠纷当事人关系 (DISPUTE_PARTY)
(party:Entity {uuid: "party-001", name: "张某新", type: "Party"})
-[:RELATES_TO {
    uuid: "rel-party-case-001",
    type: "DISPUTE_PARTY",
    fact: "张某新作为申请人提起网贷利率纠纷诉讼",
    role: "申请人",
    valid_at: 1668470400000,         // 2022-11-15 受理日期
    invalid_at: null
}]->
(case:Entity {uuid: "case-001", name: "网贷利率纠纷案", type: "Dispute"})

// 2. 纠纷调解组织关系 (DISPUTE_MEDIATOR)
(case:Entity {uuid: "case-001", name: "网贷利率纠纷案"})
-[:RELATES_TO {
    uuid: "rel-case-court-001",
    type: "DISPUTE_MEDIATOR",
    fact: "深圳市南山区人民调解委员会审理此案",
    courtRole: "一审调解组织",
    valid_at: 1668470400000,
    invalid_at: null
}]->
(court:Entity {uuid: "court-002", name: "深圳市南山区人民调解委员会", type: "Mediator"})

// 3. 纠纷社会治理条文关系 (CASE_GOVERNANCE_BASIS)
(case:Entity {uuid: "case-001"})
-[:RELATES_TO {
    uuid: "rel-case-law-001",
    type: "CASE_GOVERNANCE_BASIS",
    fact: "纠纷适用《中华人民共和国民法典》第182条",
    applicableProvision: "《民法典》第680条",
    valid_at: 1668470400000,
    invalid_at: null
}]->
(law:Entity {
...
// 4. 上诉关系时序示例 (MEDIATION_AGREEMENT)
(case1:Entity {uuid: "case-001", name: "网贷利率纠纷案一审"})
-[:RELATES_TO {
    uuid: "rel-appeal-001",
    ...
    invalid_at: 1698105600000        // 2023-10-24 调解复查后失效
}]->
(case2:Entity {
    uuid: "case-002", 
    name: "网贷利率纠纷案二审",
    disputeNumber: "SZ2024-0002"
})
```
...
|---------|----------|------------|------|------|
| DISPUTE_PARTY | Party | Dispute | 当事人参与纠纷 | 申请人、被申请人、第三人 |
| DISPUTE_MEDIATOR | Dispute | Mediator | 纠纷由某调解组织审理 | 一审调解组织、二审调解组织 |
| DISPUTE_MEDIATOR | Dispute | Mediator | 调解员审理纠纷 | 审判长、审判员 |
| CASE_GOVERNANCE_BASIS | Dispute | LegalProvision | 纠纷适用某法条 | 《民法典》第680条 |
| DISPUTE_EVIDENCE | Dispute | Evidence | 纠纷证据 | 书证、物证、证人证言 |
| MEDIATION_AGREEMENT | Dispute | Dispute | 上诉关系 | 一审→二审 |
| MENTIONS | Episode | Entity/Edge | Episode 提及实体/关系 | 文书→当事人 |
| HAS_MEMBER | Community | Entity | 社区包含成员 | 纠纷社区→当事人 |
...
```
Class: Party
Instance: 张某新 (type: Party, partyRole: 申请人)
Instance: 某网络借贷平台 (type: Party, partyRole: 被申请人)
...
**Neo4j Entity节点** (社会治理领域示例):
```json
{
  "uuid": "party-001",
  "graph_id": "social-governance-graph",
  "name": "张某新",
...
**社会治理实体节点类型示例**:
```
Party (当事人): 张某新、某网络借贷平台
Dispute (纠纷): 网贷利率纠纷案、上诉案
Mediator (调解组织): 深圳市南山区人民调解委员会、深圳市中级人民调解组织
Mediator (调解员): 张某调解员
LegalProvision (社会治理条文): 民法典第182条
Evidence (证据): 财务报表、审计报告
JudgmentDocument (调解协议书): 调解协议书、调解复查书
```
    ...
    "children": [
      {
        ...
        "children": [
          {
            "classUri": "http://social-gov.cc/ontology#NaturalPerson",
            "localName": "NaturalPerson",
            "children": [
              {
                "classUri": "http://social-gov.cc/ontology#Plaintiff",
                "localName": "Plaintiff",
                "description": "申请人",
                "children": []
              },
              {
                "classUri": "http://social-gov.cc/ontology#Defendant",
                "localName": "Defendant",
                "description": "被申请人",
                "children": []
              }
          ...
          {
            "classUri": "http://social-gov.cc/ontology#LegalPerson",
      ...
      {
        "classUri": "http://social-gov.cc/ontology#Mediator",
        "localName": "Mediator",
        "description": "调解组织",
        "children": [
          {
            "classUri": "http://social-gov.cc/ontology#SupremeMediator",
            "localName": "SupremeMediator",
            "children": []
          },
          {
            "classUri": "http://social-gov.cc/ontology#LocalMediator",
            "localName": "LocalMediator",
            "children": []
          }
        ]
      },
      {
        "classUri": "http://social-gov.cc/ontology#Dispute",
        "localName": "Dispute",
        "description": "社会治理诉讼纠纷",
        "children": [
          {
            "classUri": "http://social-gov.cc/ontology#CivilDispute",
            "localName": "CivilDispute",
            "description": "民事纠纷",
            "children": []
          },
          {
            "classUri": "http://social-gov.cc/ontology#CriminalDispute",
            "localName": "CriminalDispute",
            "description": "刑事纠纷",
            "children": []
          }
        ]
...
**核心作用**:
1. **存储原始文本**: 新闻报道、社会治理条文、对话记录、会议纪等
2. **保留上下文**: 完整保存数据的原始语境,支持溯源
3. **知识抽取源**: LLM从Episode中抽取Entity和Edge
4. **增量更新**: 新数据到来时,只处理新Episode,不影响已有数据

**类比理解** (社会治理领域示例):
- **Episode** = 调解协议书、社会治理条文、合同文本
- **Entity** = 从文书中提取的当事人(张某新)、公司(某网络借贷平台)、调解组织(深圳市南山区人民调解委员会)
- **Edge** = 从文书中提取的关系(DISPUTE_PARTY、DISPUTE_MEDIATOR、CASE_GOVERNANCE_BASIS)

#### 4.1.2 EpisodeType元数据分类

**EpisodeType**定义Episode的业务分类体系 (社会治理领域示例):

```
调解协议书 (MEDIATION_AGREEMENT)
├─ 民事判决
│  ├─ 一审民事判决
│  ├─ 二审民事判决
│  └─ 再审民事判决
...
**社会治理领域EpisodeType定义示例**:
```sql
INSERT INTO episode_type (id, name, local_name, parent_id, description, domain_hint, status) VALUES
(1, '调解协议书', 'MEDIATION_AGREEMENT', NULL, '调解组织作出的调解协议书', 'GOVERNANCE', 'ACTIVE'),
(2, '民事判决', 'CIVIL_JUDGMENT', 1, '民事纠纷的一审或调解复查', 'GOVERNANCE', 'ACTIVE'),
(3, '一审民事判决', 'FIRST_INSTANCE_CIVIL', 2, '基层或中级人民调解组织作出的一审民事判决', 'GOVERNANCE', 'ACTIVE'),
(4, '二审民事判决', 'SECOND_INSTANCE_CIVIL', 2, '高级人民调解组织作出的二审民事判决', 'GOVERNANCE', 'ACTIVE'),
(10, '社会治理条文', 'GOVERNANCE_PROVISION', NULL, '社会治理法规条文', 'GOVERNANCE', 'ACTIVE'),
(11, '社会治理', 'LAW', 10, '全国人大及其常委会制定的社会治理', 'GOVERNANCE', 'ACTIVE'),
(20, '纠纷材料', 'CASE_MATERIAL', NULL, '纠纷相关的起诉状、答辩状、证据等', 'GOVERNANCE', 'ACTIVE'),
(21, '起诉状', 'COMPLAINT', 20, '申请人提交的起诉状', 'GOVERNANCE', 'ACTIVE');
```
...
```json
{
  "uuid": "episode-001",
  "graph_id": "social-governance-graph",
  "name": "张某新与某网络借贷平台网贷利率纠纷案调解协议书",
  "source": "MEDIATION_RECORD",
  "content": "深圳市南山区人民调解委员会民事判决书SZ2024-0001。申请人张某新诉被申请人某网络借贷平台网贷利率纠纷一案...",
  "episode_type": "FIRST_INSTANCE_CIVIL",
  "legal_process": "调解程序",
  "stage_label": "调解协议",
  "case_number": "SZ2024-0001",
  "court_name": "深圳市南山区人民调解委员会",
...
**关键字段**:
- `uuid`: Episode唯一标识
- `source`: 数据来源(MEDIATION_RECORD/GOVERNANCE_PROVISION/CONTRACT等)
- `content`: 调解协议书原文或社会治理条文原文
- `episode_type`: 业务分类(对应episode_type.local_name)
- `case_number`: 纠纷编号(社会治理领域特有)
- `court_name`: 调解组织名称(社会治理领域特有)
...
Episode (一审调解协议书)
   │
   ├─[LLM抽取]→ Entity: 张某新 (type: Party, partyRole: 申请人)
   │              Entity: 某网络借贷平台 (type: Party, partyRole: 被申请人)
   │              Entity: 深圳市南山区人民调解委员会 (type: Mediator)
   │              Entity: 网贷利率纠纷案 (type: Dispute)
   │              Entity: 民法典第182条 (type: LegalProvision)
   │
   ├─[LLM抽取]→ Edge: (张某新)-[DISPUTE_PARTY]->(网贷利率纠纷案)
   │              (网贷利率纠纷案)-[DISPUTE_MEDIATOR]->(深圳市南山区人民调解委员会)
   │              (网贷利率纠纷案)-[CASE_GOVERNANCE_BASIS]->(民法典第182条)
   │
   └─[MENTIONS关系]→ Episode提及了上述所有Entity和Edge
```

**为什么需要Episode?** (社会治理领域视角)
- **可追溯性**: 知道某个当事人实体是从哪份调解协议书中提取的
- **质量评估**: 可以对比一审和二审文书抽取结果的差异
- **上下文保留**: 调解协议书原文永远保存,支持重新抽取或补充抽取
- **时效性管理**: 调解协议被二审改判后,可以通过invalid_at标记失效
...
**类型示例** (社会治理领域):
```
Party (当事人): 张某新、某网络借贷平台、第三人张某
Mediator (调解组织): 深圳市南山区人民调解委员会、深圳市中级人民调解组织、最高人民调解组织
Dispute (纠纷): 网贷利率纠纷案、上诉案、再审案
Mediator (调解员): 张某审判长、李某审判员
LegalProvision (社会治理条文): 《民法典》第680条、《民法典》第686条
Evidence (证据): 财务报表、审计报告、证人证言
JudgmentDocument (调解协议书): 调解协议书、调解复查书、再审裁定书
```
...
```json
{
  ...
  "partyRole": "申请人",
  "idNumber": "310105199001011234",
  "summary": "纠纷申请人方,自然人当事人,持有公司10%股权",
  "embedding": [0.1, 0.2, ...],
  "valid_at": 1668470400000,
  "invalid_at": null,
...
```json
{
  ...
  "name": "深圳市南山区人民调解委员会",
  "type": "Mediator",
  "courtName": "深圳市南山区人民调解委员会",
  "courtLevel": "基层人民调解组织",
  "summary": "本案一审调解组织,作出(2022)沪0105民初21387号民事判决",
  "embedding": [0.2, 0.3, ...],
  "valid_at": 1668470400000,
  "invalid_at": null
}
...
**关键字段**:
- `uuid`: 实体唯一标识
- `type`: 实体类型(对应OntClass.localName,如Party/Mediator/Dispute)
- `summary`: 实体摘要(LLM从调解协议书中生成)
- `embedding`: 向量嵌入(用于语义搜索相似纠纷)
- `valid_at/invalid_at`: 时序管理(如调解协议被二审改判后失效)
...
Neo4j Entity.type = "Party"
   ↓ 查找
关系数据库 ont_class WHERE local_name = "Party" AND graph_id = 'social-governance-graph'
   ↓ 获取
本体定义: 父类(LegalEntity)、属性(partyName/partyType/partyRole)、约束
   ↓ 验证
Entity的属性是否符合本体定义(如partyRole必须是:申请人/被申请人/第三人)
```
...
| `DISPUTE_EVIDENCE` | Dispute | Evidence | 纠纷证据 | 书证、物证、证人证言 |
| `MEDIATION_AGREEMENT` | Dispute | Dispute | 上诉关系 | 一审→二审 |
| `MENTIONS` | Episode | Entity/Edge | Episode提及实体/关系 | 调解协议书→当事人 |
| `HAS_MEMBER` | Community | Entity | 社区包含成员 | 纠纷社区→当事人 |
...
**分类维度** (社会治理领域示例):
```java
domain_type: "社会治理"                    // 领域类型
community_type: "纠纷簇"               // 社区类型(民法典纠纷簇/劳动争议纠纷簇)
region: "REGION_SHANGHAI"              // 地域(上海地区)
scenario_type: "网贷利率纠纷"           // 场景类型
legal_process: "调解程序"               // 社会治理流程
court_level: "基层人民调解组织"             // 调解组织级别
```

**社会治理社区元数据示例**:
```sql
INSERT INTO community (id, graph_id, name, summary, domain_type, community_type, region, scenario_type) VALUES
(1, 'social-governance-graph', '上海地区网贷利率纠纷纠纷社区', 
 '包含上海地区2022-2024年网贷利率纠纷相关纠纷、当事人、调解组织和社会治理条文',
 '社会治理', '纠纷簇', 'REGION_SHANGHAI', '网贷利率纠纷'),
(2, 'social-governance-graph', '劳动合同法相关实体社区',
 '包含劳动合同法相关法条、劳动争议案例、劳动监察部门',
 '社会治理', '法条簇', 'REGION_CN', '劳动争议');
```
...
│  - 节点数: 56                                           │
│  - 纠纷: 网贷利率纠纷案一审、二审                         │
│  - 当事人: 张某新(申请人)、某网络借贷平台(被申请人)     │
│  - 调解组织: 深圳市南山区人民调解委员会、深圳市中级人民调解组织     │
│  - 法条: 《民法典》第680条、《民法典》第686条            │
│  - 调解员: 张某审判长、李某审判员                          │
│  - 证据: 财务报表、审计报告、股东会决议                   │
├─────────────────────────────────────────────────────────┤
│ 社区2: "民法典司法解释相关实体社区"                       │
...
**用途** (社会治理领域视角):
- **类案推荐**: 浏览"网贷利率纠纷"社区,快速找到相似纠纷和适用法条
- **知识发现**: 发现同一调解员审理的多个纠纷,分析调解倾向
- **社会治理推荐**: 浏览某纠纷时,推荐相关的司法解释和类似案例
- **调解规律分析**: 通过社区发现同一调解组织对某类纠纷的调解标准
...
**社会治理纠纷上诉时序示例**:
```
2022-11-15: (张某新)-[DISPUTE_PARTY {role: "申请人", valid_at: 2022-11-15, invalid_at: null}]->(网贷利率纠纷案一审)
            (网贷利率纠纷案一审)-[DISPUTE_MEDIATOR {courtRole: "一审调解组织", valid_at: 2022-11-15, invalid_at: null}]->(深圳市南山区人民调解委员会)

2023-10-24: 调解协议后,部分关系失效
            (网贷利率纠纷案一审)-[DISPUTE_MEDIATOR {courtRole: "一审调解组织", valid_at: 2022-11-15, invalid_at: 2023-10-24}]->(深圳市南山区人民调解委员会)

2023-11-10: 提起上诉,创建二审关系
            (网贷利率纠纷案一审)-[MEDIATION_AGREEMENT {valid_at: 2023-11-10, invalid_at: null}]->(网贷利率纠纷案二审)
            (网贷利率纠纷案二审)-[DISPUTE_MEDIATOR {courtRole: "二审调解组织", valid_at: 2023-11-10, invalid_at: null}]->(深圳市中级人民调解组织)

2024-03-15: 调解复查维持原判
            (网贷利率纠纷案二审)-[DISPUTE_MEDIATOR {courtRole: "二审调解组织", valid_at: 2023-11-10, invalid_at: 2024-03-15}]->(深圳市中级人民调解组织)
```

**查询指定时间的纠纷状态** (Cypher):
```cypher
// 查询2023-12-01时,张某新的纠纷状态
MATCH (party:Party {partyName: "张某新"})
-[:DISPUTE_PARTY {valid_at: <= 1701360000000, invalid_at: > 1701360000000 OR invalid_at IS NULL}]->
(case:Dispute)
RETURN case.disputeNumber, case.caseName
```

**查询调解协议时张某新的诉讼地位** (Cypher):
```cypher
// 查询2023-05-01(一审期间),张某新的纠纷角色
MATCH (party:Party {partyName: "张某新"})
-[:DISPUTE_PARTY {valid_at: <= 1682899200000, invalid_at: > 1682899200000 OR invalid_at IS NULL}]->
(case:Dispute)
WHERE case.disputeNumber = 'SZ2024-0001'
...
- **纠纷进展追溯**: 追踪纠纷从受理、一审、二审到执行的全过程
- **法条时效性**: 某法条何时生效、何时被修订或废止
- **调解标准演变**: 同一调解组织对某类纠纷的调解标准随时间的变化
- **当事人状态管理**: 当事人诉讼地位变化(申请人→上诉人→被上诉人)
...
```json
{
  "graphId": "social-gov-kg-2024",
  "name": "社会治理知识图谱2024",
  "description": "包含中国民商事社会治理的知识图谱",
  "nodeCount": 15678,
  "edgeCount": 45234,
  "episodeCount": 892,
  "communityCount": 156,
...
**通俗理解** (社会治理领域示例):
- 社会治理知识图谱中,找出"网贷利率纠纷纠纷簇"(相关纠纷、当事人、调解组织、法条聚集在一起)
- 社会治理条文网络中,找出"民法典合同编相关法条簇"(互相引用的法条聚集在一起)
- 调解协议书网络中,找出"同一调解员审理的类似纠纷簇"(调解标准一致的纠纷)
...
// 获取社区内所有节点的摘要
List<String> summaries = getMemberSummaries(graphId, memberUuids);
// 例如: ["张某新,纠纷申请人,持有公司10%股权", "某网络借贷平台,纠纷被申请人", 
//        "网贷利率纠纷案,案号(2022)沪0105民初21387号", "《民法典》第680条,公司僵局救济"]

// LLM 合并摘要(二叉树方式高效处理大量文本)
String mergedSummary = summarizer.summarize(summaries);
// 输出: "上海地区网贷利率纠纷相关实体,包含申请人张某新、被申请人某网络借贷平台、审理调解组织深圳市南山区人民调解委员会及适用法条《民法典》第680条"

// LLM 生成社区名称
String communityName = summarizer.generateCommunityName(mergedSummary);
// 例如:"上海地区网贷利率纠纷纠纷簇"、"《民法典》司法解释相关实体"、"劳动争议纠纷群体"
```
...
│  - 纠纷: 网贷利率纠纷案一审、二审                 │
│  - 当事人: 张某新(申请人)、某网络借贷平台  │
│  - 调解组织: 上海市长宁区调解组织、上海市第一中院         │
│  - 法条: 《民法典》第680条、《民法典》第686条    │
│  - 调解员: 张某审判长、李某审判员                   │
├─────────────────────────────────────────────────┤
│ 社区: "公司治理相关法条簇"                        │
│  - 法条: 《民法典》第1-50条及相关司法解释         │
│  - 案例: 85个公司治理案例                         │
│  - 主题: 股东权利、公司决议效力、董事责任         │
├─────────────────────────────────────────────────┤
│ 社区: "劳动争议纠纷群体"                          │
...
| 场景 | 社会治理领域应用 |
|------|------|
| **社会治理知识图谱** | 发现"网贷利率纠纷纠纷簇"、"劳动合同法相关实体簇" |
| **类案推荐** | 自动发现相似纠纷群体,辅助调解员调解 |
| **社会治理推荐** | 浏览某纠纷时,推荐相关法条和类似案例 |
| **调解规律分析** | 发现同一调解组织/调解员对某类纠纷的调解标准 |
| **推荐系统** | 基于社区推荐相关内容 |
...
curl -X POST 'http://localhost:8080/api/v1/ontology/graph-001/validate/batch' \
  ...
  -d '{
    "nodes": [
      {"type": "Party", "properties": {"partyName": "张某新", "partyType": "自然人", "partyRole": "申请人"}}
    ],
    "edges": [
      {"type": "DISPUTE_PARTY", "source": "party-001", "target": "case-001", "fact": "张某新作为申请人提起网贷利率纠纷诉讼"}
    ]
  }'
```
...
```java
// 本体定义
Class: LegalEntity (社会治理实体 - 根类)
  └─ Party (当事人)
      └─ NaturalPerson (自然人)
          └─ Plaintiff (申请人)

// 实例
Individual: 张某新 type: Plaintiff

// 推理结果(自动推导)
张某新 type: Plaintiff        // 显式声明
张某新 type: NaturalPerson    // ✅ 推理得出
张某新 type: Party            // ✅ 推理得出
张某新 type: LegalEntity      // ✅ 推理得出
```
...
```java
// 本体定义
ObjectProperty: DISPUTE_PARTY (当事人参与纠纷)
  InverseOf: CASE_HAS_PARTY (纠纷包含当事人)

// 显式声明
(张某新)-[DISPUTE_PARTY]->(网贷利率纠纷案)

// 推理结果
✅ (网贷利率纠纷案)-[CASE_HAS_PARTY]->(张某新)  // 自动推导
```
...
```java
// 本体定义
ObjectProperty: MEDIATION_AGREEMENT (上诉关系) 
  Characteristics: Transitive (传递性)

// 显式声明
(一审纠纷)-[MEDIATION_AGREEMENT]->(二审纠纷)
(二审纠纷)-[MEDIATION_AGREEMENT]->(再审纠纷)

// 推理结果
✅ (一审纠纷)-[MEDIATION_AGREEMENT]->(再审纠纷)  // 自动推导传递关系
```
...
```java
// 本体定义
ObjectProperty: EQUIVALENT_CASE (等效纠纷)
  Characteristics: Symmetric (对称性)

// 显式声明
(纠纷A)-[EQUIVALENT_CASE]->(纠纷B)

// 推理结果
✅ (纠纷B)-[EQUIVALENT_CASE]->(纠纷A)  // 自动推导对称关系
```
...
// 本体定义
Class: Plaintiff (申请人)
  EquivalentTo: Person AND (hasRole value "申请人")
...
// 本体定义
Class: Party (当事人)
  DisjointWith: Mediator (调解组织)

// 错误检测
❌ 某实体不能同时是 Party 和 Mediator
// 推理机会报告一致性冲突
```
...
public List<String> inferApplicableLaws(String caseUri) {
    InfModel infModel = infModelCache.get("social-gov-kg");
    
    String sparql = """
        PREFIX legal: <http://social-gov.cc/ontology#>
        
        SELECT ?law WHERE {
          <%s> legal:CASE_GOVERNANCE_BASIS ?law .
          
          # 推理: 如果纠纷属于民事纠纷,则适用民法典
          <%s> legal:disputeCategory "民事纠纷" .
...
```java
// 场景: 基于推理的类案推荐
public List<String> recommendSimilarDisputes(String caseUri) {
    InfModel infModel = infModelCache.get("social-gov-kg");
    
    // 推理: 同一调解员审理的类似纠纷可能有相似调解
    String sparql = """
        PREFIX legal: <http://social-gov.cc/ontology#>
        
        SELECT ?similarDispute WHERE {
          <%s> legal:DISPUTE_MEDIATOR ?judge .
          ?similarDispute legal:DISPUTE_MEDIATOR ?judge .
          ?similarDispute legal:disputeCategory <%s>.legal:disputeCategory .
          
          # 排除自身
          FILTER(?similarDispute != <%s>)
        }
...
{
  "name": "社会治理知识图谱本体",
  "namespace": "http://social-gov.cc/ontology",
  "version": "1.0.0",
  "description": "民法典领域本体定义"
}
```
...
const validationResult = await ontologyApi.validateBatch('graph-001', {
  nodes: [{ type: 'Party', properties: { partyName: '张某新', partyType: '自然人', partyRole: '申请人' } }],
  edges: []
})
...
```bash
# 1. 执行社区检测
curl -X POST 'http://localhost:8080/api/v1/graph/social-gov-kg/communities/build' \
  -H 'Authorization: Bearer <token>'

# 2. 查询社区列表
curl -X GET 'http://localhost:8080/api/v1/graph/social-gov-kg/communities'
```
...
```bash
# 查询2023年5月的上下文
curl -X GET 'http://localhost:8080/api/v1/graph/social-gov-kg/temporal/query' \
  -H 'Content-Type: application/json' \
  -d '{
    "graphId": "social-gov-kg",
...
```bash
# 1. 检查节点数
curl -X GET 'http://localhost:8080/api/v1/graph/social-gov-kg/nodes/count'

# 2. 检查边
curl -X GET 'http://localhost:8080/api/v1/graph/social-gov-kg/edges'
...
### 10.3 实战案例:社会治理知识图谱上下文工程

#### 10.3.1 场景描述

**背景**: 某律师事务所需要构建"民法典"知识图谱,用于辅助律师快速查询网贷利率纠纷相关的社会治理条文、案例和当事人关系。

**主线案例**: 张某新与某网络借贷平台网贷利率纠纷案（案号:SZ2024-0001）

**需求**:
1. 导入调解协议书(调解协议书、调解复查书)
2. 自动抽取实体(当事人张某新、公司某网络借贷平台、调解组织深圳市南山区人民调解委员会)
3. 构建关系网络(DISPUTE_PARTY当事人参与、DISPUTE_MEDIATOR纠纷审理、CASE_GOVERNANCE_BASIS社会治理依据)
4. 支持时间线查询(纠纷何时受理、何时调解协议、何时上诉)
5. 发现关联案例(类似网贷利率纠纷纠纷、适用相同法条的案例)
6. 为LLM提供精准上下文,辅助社会治理分析和类案推荐
...
```bash
# 1. 创建本体定义
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/definition' \
  -H 'Authorization: Bearer <token>' \
  -H 'Content-Type: application/json' \
  -d '{
    "name": "社会治理知识图谱本体",
...
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/classes' \
  ...
  -d '{
    ...
    "parentClassId": 1,
    "description": "调解组织",
    "example": "深圳市南山区人民调解委员会、深圳市中级人民调解组织"
  }'
...
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/classes' \
  ...
  -d '{
    ...
    "parentClassId": 1,
    "description": "社会治理诉讼纠纷",
    "example": "SZ2024-0001网贷利率纠纷案"
  }'

# 创建子类: LegalProvision(社会治理条文)
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/classes' \
  -H 'Content-Type: application/json' \
  -d '{
    "localName": "LegalProvision",
    "parentClassId": 1,
    "description": "社会治理法规条文",
    "example": "《民法典》第680条"
  }'
```
...
LegalEntity (社会治理实体)
  ├─ Party (当事人)
  │   ├─ NaturalPerson (自然人) - 如: 张某新
  │   └─ LegalPerson (法人) - 如: 某网络借贷平台
  ├─ Mediator (调解组织)
  │   ├─ LocalMediator (地方调解组织) - 如: 深圳市南山区人民调解委员会
  │   └─ IntermediateMediator (中级调解组织) - 如: 深圳市中级人民调解组织
  ├─ Dispute (纠纷)
  │   ├─ CivilDispute (民事纠纷) - 如: 网贷利率纠纷案
  │   └─ CriminalDispute (刑事纠纷)
  └─ LegalProvision (社会治理条文)
      ├─ Law (社会治理) - 如: 《民法典》
...
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/properties' \
  ...
  -d '{
    ...
    "rangeClassId": 5,   // Dispute
    "description": "涉及某社会治理案例"
  }'
```
...
curl -X POST 'http://localhost:8080/api/v1/graphiti/data/add' \
  ...
  -d '{
    ...
    "name": "2024年民法典修订新闻",
    "source": "NEWS_ARTICLE",
    "content": "2024年3月15日,全国人大常委会通过了新的民法典修订案,注册资本实缴期限由5年调整为3年...",
    "episodeType": "GOVERNANCE_AMENDMENT",
    "legalProcess": "立法程序",
    "stageLabel": "审议通过"
  }'
```
...
curl -X POST 'http://localhost:8080/api/v1/graphiti/data/batch' \
  ...
  -d '{
    ...
    "episodes": [
      ...
      {
        "name": "民法典第182条",
        "source": "GOVERNANCE_PROVISION",
        "content": "《中华人民共和国民法典》第182条:公司经营管理发生严重困难,继续存续会使股东利益受到重大损失...",
        "episodeType": "LAW"
      }
    ]
  }'
...
print(result.edges)
# [
#   {"source": "全国人大常委会", "target": "民法典", "type": "AMENDED"},
#   {"source": "民法典", "target": "注册资本", "type": "REGULATES"}
# ]
```
...
```bash
# 创建当事人: 张某新
curl -X POST 'http://localhost:8080/api/v1/graph/social-gov-kg/nodes' \
  -H 'Content-Type: application/json' \
  -d '{
    "name": "张某新",
    "type": "Party",
...
curl -X POST 'http://localhost:8080/api/v1/graph/social-gov-kg/nodes' \
  ...
  -d '{
    "name": "网贷利率纠纷案",
    "type": "Dispute",
    "disputeNumber": "SZ2024-0001",
    "disputeCategory": "民事纠纷",
    "summary": "张某新诉某网络借贷平台网贷利率纠纷案",
    "valid_at": 1668470400000
  }'
```
...
```bash
# 张某新 DISPUTE_PARTY 网贷利率纠纷案
curl -X POST 'http://localhost:8080/api/v1/graph/social-gov-kg/edges' \
  -H 'Content-Type: application/json' \
  -d '{
...
**场景**: 律师需要分析"张某新与某网络借贷平台网贷利率纠纷案",要求AI提供社会治理意见。

**步骤1: 提取内容上下文**

```bash
# 查询与张某新相关的所有实体
curl -X GET 'http://localhost:8080/api/v1/graph/social-gov-kg/nodes?relatedTo=party-001'

# 查询张某新所属的社区
curl -X GET 'http://localhost:8080/api/v1/graph/social-gov-kg/communities?memberUuid=party-001'
```
...
context = {
    ...
    "related_entities": [
        {"name": "张某新", "type": "Party", "partyRole": "申请人", "partyType": "自然人"},
        {"name": "某网络借贷平台", "type": "Party", "partyRole": "被申请人", "partyType": "法人"},
        {"name": "深圳市南山区人民调解委员会", "type": "Mediator", "courtLevel": "基层人民调解组织"},
        {"name": "《民法典》第680条", "type": "LegalProvision", "relevance": "高"}
    ],
    "timeline": [
        {"date": "2022-11-15", "event": "张某新提起网贷利率纠纷诉讼"},
        {"date": "2023-10-24", "event": "深圳市南山区人民调解委员会调解协议"},
        {"date": "2023-11-10", "event": "张某新不服判决,提起上诉"},
        {"date": "2024-03-15", "event": "深圳市中级人民调解组织调解复查"}
    ],
    "similar_cases": [
        {"name": "李某诉上海某科技公司网贷利率纠纷案", "similarity": 0.92, "disputeNumber": "（2021）沪0115民初12345号"},
        {"name": "王某诉杭州某电商公司网贷利率纠纷案", "similarity": 0.87, "disputeNumber": "（2022）浙0106民初6789号"}
    ],
    "applicable_laws": [
        {"name": "《民法典》第680条", "content": "公司经营管理发生严重困难,继续存续会使股东利益受到重大损失"},
        {"name": "《民法典》第686条", "content": "当事人一方不履行非金钱债务或者履行非金钱债务不符合约定的"}
    ]
}
```
...
# 构建LLM提示词
prompt = f"""
你是一位资深民法典律师,请根据以下上下文信息,为张某新提供社会治理意见:
...
1. **张某新的诉求合理性**: ✅ 合理
   - 张某新持有公司10%股权,符合《民法典》第680条的股东资格
...
```bash
# 批量验证节点和边
curl -X POST 'http://localhost:8080/api/v1/ontology/social-gov-kg/validate/batch' \
  -H 'Content-Type: application/json' \
  -d '{
    "nodes": [
      {
