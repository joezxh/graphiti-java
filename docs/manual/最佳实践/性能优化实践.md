...
## 项目结构
ontograph-java 采用模块化分层架构：前端 Vue3、后端 Spring Boot、核心业务模块 graphiti-module-core、框架基础设施 graphiti-framework、Neo4j 图数据库、PostgreSQL/MariaDB 元数据存储、Redis 缓存。核心算法与性能优化主要集中在 graphiti-module-core 的 util 包与 service 实现类中。
