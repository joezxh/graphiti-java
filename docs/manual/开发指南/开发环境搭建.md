...
## 简介
本指南面向OntoGraph的开发者，提供从零开始的本地开发环境搭建步骤，涵盖JDK 21、Node.js、数据库（MySQL/PostgreSQL）与图数据库（Neo4j）、缓存（Redis）以及IDE（IntelliJ IDEA、VS Code）的配置要点；同时给出Maven多模块项目的导入与构建流程、Docker容器化本地开发环境配置、环境变量与数据库初始化、项目启动步骤及常见问题排查方案。

## 项目结构
OntoGraph采用多模块Maven工程组织，包含框架层、业务模块、服务入口与前端工程，并提供Docker一键编排开发环境。

```mermaid
graph TB
subgraph "后端"
S["ontograph-server<br/>Spring Boot 入口"]
F["ontograph-framework<br/>通用/安全/MyBatis/Redis"]
M1["ontograph-module-system<br/>系统管理"]
M2["ontograph-module-core<br/>知识图谱核心"]
end
subgraph "前端"
...
```mermaid
graph TB
FE["浏览器/前端<br/>ontograph-web"] --> API["后端API<br/>ontograph-server"]
...
- [ontograph-web/package.json:22-30](file://ontograph-web/package.json#L22-L30)
...
- [ontograph-server/src/main/resources/application-dev.yml:488-502](file://ontograph-server/src/main/resources/application-dev.yml#L488-L502)
...
章节来源
- [ontograph-server/src/main/resources/application-dev.yml:503-506](file://ontograph-server/src/main/resources/application-dev.yml#L503-L506)
...
章节来源
- [ontograph-server/src/main/resources/application-dev.yml:11-676](file://ontograph-server/src/main/resources/application-dev.yml#L11-L676)
...
- [ontograph-framework/graphiti-spring-boot-starter-security/src/main/java/com/graphiti/framework/security/config/SecurityConfig.java:116-136](file://ontograph-framework/graphiti-spring-boot-starter-security/src/main/java/com/graphiti/framework/security/config/SecurityConfig.java#L116-L136)
...
### Docker容器化本地开发环境
- docker-compose编排：ontograph-java应用、PostgreSQL、Redis。
...
- 前端开发：在ontograph-web目录打开终端，使用pnpm命令进行安装与开发。
...
#### 方式一：本地原生启动
- 后端：在ontograph-server目录执行spring-boot:run或IDE运行GraphitiApplication。
- 前端：在ontograph-web目录执行pnpm dev，访问http://localhost:5173。
- Swagger：http://localhost:8080/swagger-ui.html

章节来源
- [README.md:293-312](file://README.md#L293-L312)
- [ontograph-server/src/main/resources/application.yml:58-67](file://ontograph-server/src/main/resources/application.yml#L58-L67)
...
```mermaid
graph LR
Root["根POM<br/>pom.xml"] --> FW["ontograph-framework"]
...
- [ontograph-server/src/main/resources/application-dev.yml:498-501](file://ontograph-server/src/main/resources/application-dev.yml#L498-L501)
- [ontograph-web/vite.config.ts:18-26](file://ontograph-web/vite.config.ts#L18-L26)
...
## 结论
通过本指南，您可以在本地快速搭建OntoGraph的开发与测试环境，完成数据库与图数据库初始化、LLM提供商配置、前后端联调与容器化部署。遇到问题时，可依据故障排查章节逐项定位并解决。
