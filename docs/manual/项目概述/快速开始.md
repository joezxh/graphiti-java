...
## 简介
本指南面向首次接触 Graphiti-Java 的开发者，帮助你在约 30 分钟内完成环境准备、数据库初始化、应用配置与启动，并验证基础功能（创建图谱、添加数据、混合检索、本体定义等）。项目采用前后端分离架构：后端基于 Spring Boot 3.5.5，前端基于 Vue 3 + Vite，数据层包含 Neo4j（图数据库）、PostgreSQL（元数据与系统数据）以及 Redis（缓存）。
...
## 结论
按照本指南完成环境准备、数据库初始化与配置后，你可以在本地快速启动 Graphiti-Java，并通过 Swagger 与 curl 验证核心功能。若需进一步扩展，可参考 README 中的技术栈与模块结构，逐步接入更多 LLM 提供商与数据源。
