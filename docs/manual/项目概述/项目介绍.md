...
- [三国人物关系图谱示例.json](file://ontograph-frontend/三国人物关系图谱示例.json)
- [法律知识图谱示例.json](file://ontograph-frontend/法律知识图谱示例.json)
</cite>-->
...
## 简介
ontograph-java 是一个面向 Java 生态的“时序知识图谱后端系统”，专注于将时序推理、LLM 驱动的实体关系抽取、混合检索与本体治理能力整合到统一平台。它通过 Neo4j 存储图结构与向量索引，结合 PostgreSQL/MySQL 管理元数据与系统数据，提供从“数据导入—抽取—验证—检索—治理”的完整闭环。
...
## 项目结构
ontograph-java 采用多模块分层架构，核心模块包括：
- ontograph-server：Spring Boot 启动入口，承载 API 与配置。
- ontograph-module-core：核心业务模块，包含知识图谱 CRUD、导入/抽取、检索、本体验证、社区发现、时序管理、数据质量等。
- ontograph-module-system：系统管理模块，提供用户/角色/菜单/认证等基础能力。
- ontograph-framework：框架基础设施，封装通用组件（MyBatis、Redis、安全等）。
- ontograph-frontend：前端（Vue 3），提供可视化界面与交互。

```mermaid
graph TB
subgraph "后端"
S["ontograph-server<br/>Spring Boot 入口"]
C["ontograph-module-core<br/>核心业务"]
SYS["ontograph-module-system<br/>系统管理"]
F["ontograph-framework<br/>基础设施"]
end
subgraph "存储"
N["Neo4j<br/>图数据库"]
...
subgraph "前端"
W["ontograph-frontend<br/>Vue 3 应用"]
end
W --> S
S --> C
S --> SYS
...
- [CommunityServiceImpl.java:42-61](file://ontograph-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/CommunityServiceImpl.java#L42-L61)
- [TemporalServiceImpl.java:27-41](file://ontograph-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/TemporalServiceImpl.java#L27-L41)
...
- [QwenEmbedderServiceImpl.java:24-65](file://ontograph-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/ai/QwenEmbedderServiceImpl.java#L24-L65)

## 架构总览
系统采用“前后端分离 + 多存储”的架构：前端通过 REST API 与后端交互；后端以 Spring Boot 为核心，核心业务在 ontograph-module-core 中实现，Neo4j 负责图存储与向量索引，PostgreSQL/MySQL 负责元数据与系统数据，Redis 负责缓存与会话。

```mermaid
graph TB
FE["ontograph-web<br/>Vue 3 前端"]
API["ontograph-server<br/>Spring Boot API"]
CORE["ontograph-module-core<br/>业务服务"]
SYS["ontograph-module-system<br/>系统服务"]
FW["ontograph-framework<br/>基础设施"]
DB["Neo4j<br/>图数据库"]
...
**章节来源**
- [SearchServiceImpl.java:94-148](file://ontograph-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/SearchServiceImpl.java#L94-L148)
- [SearchServiceImpl.java:227-308](file://ontograph-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/SearchServiceImpl.java#L227-L308)
...
**图表来源**
- [CommunityServiceImpl.java:42-61](file://ontograph-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/CommunityServiceImpl.java#L42-L61)
- [CommunityServiceImpl.java:137-180](file://ontograph-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/CommunityServiceImpl.java#L137-L180)
...
**图表来源**
- [TemporalServiceImpl.java:44-85](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/TemporalServiceImpl.java#L44-L85)
- [TemporalServiceImpl.java:88-107](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/TemporalServiceImpl.java#L88-L107)
...
**图表来源**
- [EntityDedupServiceImpl.java:54-172](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/EntityDedupServiceImpl.java#L54-L172)
- [EntityDedupServiceImpl.java:240-312](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/EntityDedupServiceImpl.java#L240-L312)
...
**章节来源**
- [OpenAiLlmClientServiceImpl.java:36-110](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/ai/OpenAiLlmClientServiceImpl.java#L36-L110)
- [QwenEmbedderServiceImpl.java:24-65](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/ai/QwenEmbedderServiceImpl.java#L24-L65)
...
- 模块依赖
  - ontograph-server 依赖 ontograph-module-core、ontograph-module-system、ontograph-framework。
  - ontograph-module-core 依赖 ontograph-framework 与 Neo4j/数据库驱动。
  - ontograph-framework 提供 MyBatis、Redis、安全等通用能力。

```mermaid
graph LR
SB["Spring Boot"] --> MOD_CORE["ontograph-module-core"]
SB --> MOD_SYS["ontograph-module-system"]
SB --> FW["ontograph-framework"]
...
**章节来源**
- [OpenAiLlmClientServiceImpl.java:42-53](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/ai/OpenAiLlmClientServiceImpl.java#L42-L53)
- [QwenEmbedderServiceImpl.java:29-39](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/ai/QwenEmbedderServiceImpl.java#L29-L39)
- [SearchServiceImpl.java:227-284](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/SearchServiceImpl.java#L227-L284)
- [OntologyValidationServiceImpl.java:256-322](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/OntologyValidationServiceImpl.java#L256-L322)
- [TemporalServiceImpl.java:44-85](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/TemporalServiceImpl.java#L44-L85)
- [CommunityServiceImpl.java:117-126](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/CommunityServiceImpl.java#L117-L126)
- [EntityDedupServiceImpl.java:268-312](file://ontograph-module-core/src/main/java/com/raphiti/module/graphiti/service/impl/EntityDedupServiceImpl.java#L268-L312)

## 结论
ontograph-java 以“时序+LLM+混合检索+本体治理+社区发现”为核心能力，为企业级知识图谱建设提供了从数据导入到检索治理的一体化解决方案。其模块化设计与多存储架构使其具备良好的扩展性与可维护性；通过严格的本体检错与去重策略，保障了知识质量；通过原生时序管理与丰富的检索模式，提升了用户体验与业务价值。随着前端控制台与本体增强计划的推进，ontograph-java 正朝着更易用、更智能、更合规的方向演进。
