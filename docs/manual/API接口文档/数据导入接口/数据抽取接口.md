...
## 简介
本文件为 Graphiti-Java 的数据抽取接口专业参考，覆盖两类核心能力：
...
## 结论
Graphiti-Java 的数据抽取接口以 LLM 为核心，通过清晰的控制器-服务-模型分层，提供了通用与法律两大领域的抽取能力。接口支持批量、增量与自定义规则，具备完善的错误处理与性能控制策略，适合在生产环境中构建高质量的知识图谱。
