...
## 简介
本文件为 Graphiti-Java 知识图谱管理接口的权威参考，覆盖图谱生命周期管理（创建、查询、更新、删除、清空、克隆）、节点与边的 CRUD、图谱统计、本体管理、检索与记忆、时序历史查询等能力。文档提供端点清单、请求参数、响应格式、错误码定义，并结合数据模型与查询优化策略给出实践建议与示例。
...
## 结论
Graphiti-Java 提供了覆盖知识图谱全生命周期与高级能力的 REST 接口体系，统一的响应封装与完善的参数校验保障了接口的一致性与可靠性。建议在生产环境中结合分页、缓存与异步策略优化性能，并通过本体推理与一致性检查确保数据质量。
