...
## 项目结构
ontograph-java 采用前后端分离与多模块架构：前端 Vue 3 应用位于 graphiti-web，后端 Spring Boot 应用位于 graphiti-server，核心业务逻辑在 graphiti-module-core，系统管理在 graphiti-module-system，框架基础设施在 graphiti-framework。数据库方面，PostgreSQL/MySQL 存放元数据与系统数据，Neo4j 存放知识图谱，Redis 作为缓存与会话存储。
