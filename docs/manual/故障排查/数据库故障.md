...
## 简介
本文件面向Graphiti-Java项目的数据库运维与开发人员，聚焦于数据库故障排查与治理。内容涵盖：
...
## 项目结构
Graphiti-Java采用“后端服务 + 关系数据库(PostgreSQL) + 图数据库(Neo4j)”的组合架构。数据库相关配置集中在应用配置文件中，并通过SQL脚本完成初始化与迁移。
...
- [init.cypher:1-33](file://sql/neo4j/init.cypher#L1-L33)
- [DataImportServiceImpl.java:213-220](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/DataImportServiceImpl.java#L213-L220)
...
- 连接超时
  ...
  - 参考：[application-dev.yml:487-676](file://graphiti-server/src/main/resources/application-dev.yml#L487-L676)、[application-prod.yml:10-41](file://config/application-prod.yml#L10-L41)
- 死锁
...
- 关系类型一致性
  ...
  - 参考：[2026-05-11-neo4j-relation-type-consistency.md:1-141](file://docs/superpowers/plans/2026-05-11-neo4j-relation-type-consistency.md#L1-L141)、[DataImportServiceImpl.java:213-220](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/service/impl/DataImportServiceImpl.java#L213-L220)
...
## 结论
通过规范化的配置、完善的脚本与严格的监控，Graphiti-Java能够在PostgreSQL与Neo4j环境下稳定运行。遇到故障时，建议优先从连接与索引入手排查，结合迁移脚本与初始化脚本进行一致性验证，并制定清晰的回滚与恢复流程，确保业务连续性。

## 附录
- 关键配置与脚本路径
  - 开发配置：[application-dev.yml:487-676](file://graphiti-server/src/main/resources/application-dev.yml#L487-L676)
