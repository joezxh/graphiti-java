...
- [CommunityNodeNamespace.java](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/node/CommunityNodeNamespace.java)
- [OntologyNamespace.java](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/graph/OntologyNamespace.java)
...
## 简介
本文件面向图数据库管理员与算法开发者，系统化阐述基于 Neo4j 的图数据库设计与实现。内容涵盖图数据模型、节点与关系的命名空间设计、Cypher 查询模式与图遍历算法、初始化脚本与向量索引配置、数据生命周期与索引优化、查询性能调优、图模式设计原则、数据一致性保障与扩展性考虑，并提供可落地的 Cypher 示例与最佳实践。
...
- 命名空间层：将业务语义封装为命名空间，协调 Service 与底层 Driver，屏蔽复杂 Cypher 细节。
...
- [CommunityNodeNamespace.java:1-44](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/node/CommunityNodeNamespace.java#L1-L44)
- [OntologyNamespace.java:1-46](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/graph/OntologyNamespace.java#L1-L46)
...
## 架构总览
下图展示从命名空间到服务层再到 Neo4j 的调用链路，以及本体校验与向量索引的关键位置。

```mermaid
sequenceDiagram
participant NS_Node as "命名空间 : 实体节点"
participant NS_Edge as "命名空间 : 实体边"
participant NS_Comm as "命名空 : 社区节点"
participant Svc as "服务层(GraphNeo4jService)"
participant DB as "Neo4j"
NS_Node->>Svc : 创建实体节点(含 embedding)
Svc->>DB : 执行 Cypher(CREATE Entity)
DB-->>Svc : 返回节点
Svc-->>NS_Node : 节点信息
NS_Edge->>Svc : 创建关系边(含 embedding)
Svc->>DB : 执行 Cypher(CREATE RELATES_TO)
DB-->>Svc : 返回边
Svc-->>NS_Edge : 边信息
NS_Comm->>Svc : 构建社区(标签传播)
Svc->>DB : 查询邻接/创建社区节点
DB-->>Svc : 返回社区结果
Svc-->>NS_Comm : 社区统计
```
...
图表来源
- [EntityNodeNamespace.java:36-94](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/node/EntityNodeNamespace.java#L36-L94)
...
- [CommunityNodeNamespace.java:25-28](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/node/CommunityNodeNamespace.java#L25-L28)
...
- 命名空间职责
  - 实体节点命名空间：封装节点创建、批量导入、详情查询、边关联查询、删除与向量刷新。
  - 实体边命名空间：封装边创建、批量导入、详情查询、源目标边查询、删除与向量刷新。
  - 社区节点命名空间：封装社区构建、列表查询、搜索。
  - 本体命名空间：封装本体定义与全量信息查询。
...
实体节点命名空间 --> GraphNeo4jService : "调用"
实体边命名空间 --> GraphNeo4jService : "调用"
社区节点命名空间 --> CommunityService : "调用"
```

图表来源
- [EntityNodeNamespace.java:27-94](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/node/EntityNodeNamespace.java#L27-L94)
- [EntityEdgeNamespace.java:22-86](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/edge/EntityEdgeNamespace.java#L22-L86)
- [CommunityNodeNamespace.java:18-43](file://graphiti-module-core/src/main/java/com/graphiti/module/graphiti/namespace/node/CommunityNodeNamespace.java#L18-L43)
