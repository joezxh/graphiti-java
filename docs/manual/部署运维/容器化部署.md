...
## 简介
本文件面向OntoGraph的容器化部署，系统性阐述多阶段Docker构建流程（前端构建阶段与后端编译阶段）、运行时JRE环境与非root安全配置、健康检查机制以及JVM内存调优参数。同时提供docker-compose开发与生产环境配置的对比说明，涵盖容器网络、卷挂载策略与环境变量设置；解释容器启动流程、依赖服务连接顺序与常见故障排查方法，并给出容器镜像构建、推送与拉取的最佳实践。

## 项目结构
OntoGraph采用前后端分离的多模块Maven工程，后端为Spring Boot应用，前端为Vue 3 + Vite项目。容器化采用多阶段Dockerfile：第一阶段使用Maven与Node.js工具链完成前端打包与后端编译，第二阶段基于轻量Alpine JRE运行时镜像，复制JAR并以非root用户运行，内置健康检查与JVM参数。
...
- [GraphitiApplication.java:15-34](file://ontograph-server/src/main/java/com/graphiti/GraphitiApplication.java#L15-L34)
...
- 构建阶段要点
  ...
  - 在ontograph-web目录执行pnpm安装与构建，生成dist静态资源。
  - 将dist复制到后端resources/static目录，使JAR内嵌静态资源。
  - 在ontograph-server目录执行Maven打包，跳过测试以加速构建。
- 运行阶段要点
...
- 服务与端口映射
  - ontograph-java：开发映射8080:8080；生产未映射端口，通过反向代理或负载均衡暴露。
- 依赖服务
...
- 健康检查与重启策略
  - 开发：ontograph-java无重启策略，依赖compose管理；PostgreSQL/Redis健康检查。
...
### 容器启动流程与依赖连接
- 启动顺序
  - compose按服务定义启动，ontograph-java通过depends_on等待PostgreSQL健康，再启动Redis。
- 依赖连接
...
- [application.yml:43-55](file://ontograph-server/src/main/resources/application.yml#L43-L55)
...
- Compose依赖
  - PostgreSQL、Redis镜像，ontograph-java镜像由Dockerfile构建。
...
## 结论
OntoGraph的容器化方案通过多阶段构建将前端与后端整合为单一可执行JAR，并在Alpine JRE上以非root用户安全运行。配合docker-compose的开发与生产配置，实现了灵活的环境切换、依赖服务编排、可观测性与安全加固。遵循本文的配置与最佳实践，可在不同环境中稳定交付OntoGraph服务。
