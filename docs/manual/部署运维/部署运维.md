...
## 简介
本文件面向DevOps工程师与系统管理员，提供OntoGraph的容器化部署与生产运维实践指南。内容涵盖Docker容器化方案、生产环境配置、环境变量清单、依赖服务设置、负载均衡与高可用、灾难恢复策略、监控与日志、性能调优、安全加固、备份恢复与版本升级流程，以及运维脚本与自动化工具使用方法。

## 项目结构
OntoGraph采用多模块Maven工程组织，后端以Spring Boot应用为核心，前端为独立的Vue3项目，数据库包括PostgreSQL/MySQL与Neo4j，缓存使用Redis。Docker多阶段构建同时完成前端打包与后端JAR生成，并在运行镜像中内置健康检查与JVM调优参数。

```mermaid
graph TB
subgraph "应用层"
S["ontograph-server<br/>Spring Boot 应用"]
W["ontograph-web<br/>Vue3 前端"]
end
subgraph "框架层"
F1["ontograph-framework<br/>通用/安全/MyBatis/Redis"]
F2["ontograph-module-system<br/>系统管理"]
F3["ontograph-module-core<br/>知识图谱核心"]
end
subgraph "数据与缓存"
...
## 核心组件
- 应用入口与自动装配
  - 启动类位于ontograph-server模块，扫描com.graphiti包并按需排除部分Spring AI自动配置，避免不必要的外部依赖被激活。
- 配置体系
...
- [ontograph-framework/graphiti-spring-boot-starter-security/src/main/java/com/graphiti/framework/security/config/SecurityConfig.java:68-136](file://ontograph-framework/graphiti-spring-boot-starter-security/src/main/java/com/graphiti/framework/security/config/SecurityConfig.java#L68-L136)
...
subgraph "应用集群"
APP1["ontograph-java:1<br/>8080"]
APP2["ontograph-java:2<br/>8080"]
end
...
- 开发环境compose
  - ontograph-java: 映射8080；通过环境变量注入数据库、Redis、Neo4j、LLM Provider、JWT、日志级别等；依赖PostgreSQL健康与Redis启动。
...
participant App as "ontograph-java"
...
- 模块依赖
  - ontograph-server依赖framework、system、core模块；framework提供通用与集成组件；system与core分别负责系统管理与知识图谱业务。

```mermaid
graph LR
SB["ontograph-server"] --> FW["ontograph-framework"]
SB --> SYS["ontograph-module-system"]
SB --> CORE["ontograph-module-core"]
FW --> MP["MyBatis-Plus"]
FW --> RS["Redis"]
FW --> SEC["Security"]
```
...
## 结论
通过Docker多阶段构建与Compose编排，OntoGraph可在开发与生产环境快速落地。结合合理的资源配额、连接池与缓存策略、完善的监控与日志体系，以及严格的备份与灾备流程，可满足生产级可用性与可维护性要求。建议在上线前完成压测与演练，并持续优化JVM与数据库参数。
...
- 容器与编排
  - ontograph-java多副本、PostgreSQL/Redis持久化卷、日志卷、外部配置卷。
- 依赖服务
