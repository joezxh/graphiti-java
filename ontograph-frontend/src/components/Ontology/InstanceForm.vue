  ...
  <div class="instance-form">
    ...
    >
      ...
      <a-form-item label="UUID" name="uuid" v-if="!isNew">
        <a-input v-model:value="formData.uuid" disabled :placeholder="$t('common.systemAutoGenerated')" />
      </a-form-item>

      <a-form-item label="名称" name="name" :rules="[{ required: true, message: $t('common.pleaseEnterInstanceName') }]">
        <a-input v-model:value="formData.name" :placeholder="$t('common.pleaseEnterInstanceName')" />
      </a-form-item>

      <a-form-item label="类型" name="type" :rules="[{ required: true, message: $t('common.pleaseSelectType') }]">
        <a-select
      ...
      <template v-if="selectedClass && selectedClassProperties.length > 0">
        <a-divider orientation="left">{{ $t('common.propertyInfo') }}</a-divider>
          ...
          >
            ...
            <template v-if="prop.propertyType === 'DATATYPE'">
              ...
              <a-switch
                ...
                :checked-children="$t('common.yesLabel')"
                :un-checked-children="$t('common.noLabel')"
              />
              <!-- 数字类型 -->
              <a-input-number
            ...
            <template v-else-if="prop.propertyType === 'OBJECT'">
              <div class="object-ref-field">
                <a-input
                  v-model:value="formData.properties[prop.localName]"
                  :placeholder="$t('common.inputNodeUuid')"
                  style="flex: 1"
                />
                <a-button @click="showNodeSelector(prop)">{{ $t('common.selectNode') }}</a-button>
              </div>
            </template>
            ...
            <template #extra>
              <span class="prop-hint">
                <span v-if="prop.isRequired" class="required-hint">{{ $t('common.required') }}</span>
      ...
      <template v-if="extraProps.length > 0">
        <a-divider orientation="left">{{ $t('common.extendedProperties') }}</a-divider>
        <template v-for="prop in extraProps" :key="prop.key">
          <a-form-item :label="prop.label" :name="['properties', prop.key]">
            <a-input v-model:value="formData.properties[prop.key]" :placeholder="`${$t('common.pleaseEnter')} ${prop.label}`" />
          </a-form-item>
        </template>
      </template>
      ...
      <div class="add-extra-prop">
        <a-button type="dashed" block @click="showAddExtraProp = !showAddExtraProp">
          ...
          {{ $t('common.addExtendedProperty') }}
        </a-button>
        <div v-if="showAddExtraProp" class="extra-prop-form">
          <a-input v-model:value="newPropKey" :placeholder="$t('common.propertyName')" style="width: 120px; margin-right: 8px" />
          <a-input v-model:value="newPropValue" :placeholder="$t('common.propertyValue')" style="width: 200px; margin-right: 8px" />
          <a-button type="primary" size="small" @click="addExtraProp">{{ $t('common.add') }}</a-button>
        </div>
      </div>
    </a-form>
    ...
    >
      <div class="node-selector">
        <a-input-search
          ...
          :placeholder="$t('common.searchNodes')"
